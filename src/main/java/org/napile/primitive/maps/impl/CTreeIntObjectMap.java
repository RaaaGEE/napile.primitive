...
/**
 * A scalable concurrent {@link org.napile.primitive.maps.CNavigableIntObjectMap} implementation.
 ...
 * <em>optional</em> methods of the {@link org.napile.primitive.maps.IntObjectMap} and {@link org.napile.primitive.iterators.IntIterator}
...
public class CTreeIntObjectMap<V> extends AbstractIntObjectMap<V> implements CNavigableIntObjectMap<V>, Cloneable, java.io.Serializable
{
	...
	/**
	 * Returns a shallow copy of this <tt>CTreeIntObjectMap</tt>
	...
	static final class SubMap<V> extends AbstractIntObjectMap<V> implements CNavigableIntObjectMap<V>, Cloneable, java.io.Serializable
	{
		...
		private CTreeIntObjectMap.Node<V> loNode()
		{
			...
			if(loInclusive)
			{
				return m.findNear(lo, CTreeIntObjectMap.GT | CTreeIntObjectMap.EQ);
			}
			else
			{
				return m.findNear(lo, CTreeIntObjectMap.GT);
			}
		}
		...
		private CTreeIntObjectMap.Node<V> hiNode()
		{
			...
			if(hiInclusive)
			{
				return m.findNear(hi, CTreeIntObjectMap.LT | CTreeIntObjectMap.EQ);
			}
			else
			{
				return m.findNear(hi, CTreeIntObjectMap.LT);
			}
		}
		...
		private IntObjectPair<V> getNearEntry(int key, int rel)
		{
			...
			{ // adjust relation for direction
				if((rel & CTreeIntObjectMap.LT) == 0)
				{
					rel |= CTreeIntObjectMap.LT;
				}
				else
				{
					rel &= ~CTreeIntObjectMap.LT;
			...
			if(tooLow(key))
			{
				return ((rel & CTreeIntObjectMap.LT) != 0) ? null : lowestEntry();
			}
			if(tooHigh(key))
			{
				return ((rel & CTreeIntObjectMap.LT) != 0) ? highestEntry() : null;
			}
			for(; ;)
			{
		...
		public IntObjectPair<V> ceilingEntry(int key)
		{
			return getNearEntry(key, (CTreeIntObjectMap.GT | CTreeIntObjectMap.EQ));
		}

		public int ceilingKey(int key)
		{
			return getNearKey(key, (CTreeIntObjectMap.GT | CTreeIntObjectMap.EQ));
		}

		public IntObjectPair<V> lowerEntry(int key)
		{
			return getNearEntry(key, (CTreeIntObjectMap.LT));
		}

		public int lowerKey(int key)
		{
			return getNearKey(key, (CTreeIntObjectMap.LT));
		}

		public IntObjectPair<V> floorEntry(int key)
		{
			return getNearEntry(key, (CTreeIntObjectMap.LT | CTreeIntObjectMap.EQ));
		}

		public int floorKey(int key)
		{
			return getNearKey(key, (CTreeIntObjectMap.LT | CTreeIntObjectMap.EQ));
		}

		public IntObjectPair<V> higherEntry(int key)
		{
			return getNearEntry(key, (CTreeIntObjectMap.GT));
		}

		public int higherKey(int key)
		{
			return getNearKey(key, (CTreeIntObjectMap.GT));
		}
